...
`;
  ...
  if (!content.includes('InventoryIcons')) {
  ...
  const tabStart = content.indexOf('{/* Sub Tab Bar Navigation */}');
  if (tabStart !== -1) {
    const tabDivStart = content.indexOf('<div className="nav-tabs"', tabStart);
    if (tabDivStart !== -1) {
      const tabDivEnd = content.indexOf('</div>', tabDivStart);
      if (tabDivEnd !== -1) {
        const oldTabsBlock = content.substring(tabDivStart, tabDivEnd + 6);
        const newTabsBlock = `<div className="nav-tabs" style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '8px', borderBottom: '2px solid var(--border-color)', paddingBottom: '10px', flexWrap: 'wrap' }}>
        <button
          type="button"
        ...
        console.log("✅ Tabs replaced via offset!");
      }
    }
  }

  // 3. Replace Shop Stock KPI Cards
  const shopKpiStart = content.indexOf('{/* Stock Valuation KPI Cards */}');
  if (shopKpiStart !== -1) {
    const kpiDivStart = content.indexOf('<div style={{ display: \'grid\', gridTemplateColumns:', shopKpiStart);
    if (kpiDivStart !== -1) {
      // Find the closing </div> of the grid
      const kpiDivEnd = content.indexOf('</div>\n', kpiDivStart); // Wait, there are multiple divs inside.
      // Let's count matching divs
      let openDivs = 1;
      let ptr = kpiDivStart + 5; // skip first <div
      while (openDivs > 0 && ptr < content.length) {
        const nextOpen = content.indexOf('<div', ptr);
        const nextClose = content.indexOf('</div>', ptr);
      ...
      if (openDivs === 0) {
        const oldShopKpiBlock = content.substring(kpiDivStart, ptr);
        const newShopKpiBlock = `<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
            <InventoryKpiCard
        ...
        content = content.replace(oldShopKpiBlock, newShopKpiBlock);
        console.log("✅ Shop Stock KPI Cards replaced via offset!");
      }
    }
  }

  // 4. Replace Warehouse KPI Cards
  const whKpiStart = content.indexOf('{/* Warehouse Valuation KPI Cards */}');
  if (whKpiStart !== -1) {
    const kpiDivStart = content.indexOf('<div style={{ display: \'grid\', gridTemplateColumns:', whKpiStart);
    if (kpiDivStart !== -1) {
      let openDivs = 1;
      let ptr = kpiDivStart + 5;
      while (openDivs > 0 && ptr < content.length) {
        const nextOpen = content.indexOf('<div', ptr);
        const nextClose = content.indexOf('</div>', ptr);
        if (nextClose !== -1 && (nextOpen === -1 || nextClose < nextOpen)) {
      ...
      if (openDivs === 0) {
        const oldWhKpiBlock = content.substring(kpiDivStart, ptr);
        const newWhKpiBlock = `<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
            <InventoryKpiCard
        ...
        content = content.replace(oldWhKpiBlock, newWhKpiBlock);
        console.log("✅ Warehouse KPI Cards replaced via offset!");
      }
    }
  }
