...
const DB_FILE = path.resolve(workspaceRoot, './db_shared.json');
const BACKUP_FILE = path.resolve(workspaceRoot, './db_shared_backup.json');
const KEY_FILE = path.resolve(workspaceRoot, './firebase-key.json');

async function main() {
  console.log("🚀 Starting live cloud data pull from Firestore...");
  ...
  for (const doc of snapshot.docs) {
    ...
    freshDb[key] = {
      data: docData.data || docData, // fallback if data is direct
  ...
  console.log(`✓ Successfully updated local db_shared.json with real production data from Cloud Firestore!`);
