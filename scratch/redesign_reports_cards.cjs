...
if (fs.existsSync(path)) {
  ...
  if (!content.includes('function ReportSectionHeader')) {
    const helperDef = `
function ReportSectionHeader({ icon, title, extra }) {
  return (
...
`;
  ...
  // Define SVG icon functions/elements directly
  const iconSvgs = {
  ...
  const makeIconJSX = (key) => `<span dangerouslySetInnerHTML={{ __html: \`${iconSvgs[key]}\` }} style={{ display: 'flex', alignItems: 'center' }} />`;

  // Header 1: Bills Lookup
  content = content.replace(
    `<h3 style={{ color: 'var(--gold-primary)', fontSize: '1.05rem' }}>🔍 ໃບບິນຂາຍຊ່ວງນີ້ (Bills Lookup)</h3>`,
    `<ReportSectionHeader icon={${makeIconJSX('bills')}} title="ໃບບິນຂາຍຊ່ວງນີ້ (Bills Lookup)" />`
  );

  // Header 2: Debts Table
  content = content.replace(
    `<h3 style={{ color: 'var(--gold-primary)', fontSize: '1.05rem', margin: 0 }}>📒 ບັນຊີລູກຄ້າຕິດໜີ້ຊ່ວງນີ້ (Period Debts Table)</h3>`,
    `<ReportSectionHeader icon={${makeIconJSX('debts')}} title="ບັນຊີລູກຄ້າຕິດໜີ້ຊ່ວງນີ້ (Period Debts Table)" />`
  );

  // Header 3: Expenses Table
  content = content.replace(
    `<h3 style={{ color: 'var(--gold-primary)', fontSize: '1.05rem', margin: 0 }}>💸 ບັນທຶກລາຍຈ່າຍຊ່ວງນີ້ (Period Expenses Table)</h3>`,
    `<ReportSectionHeader icon={${makeIconJSX('expenses')}} title="ບັນທຶກລາຍຈ່າຍຊ່ວງນີ້ (Period Expenses Table)" />`
  );

  // Header 4: Returns & Refunds
  content = content.replace(
    `<h3 style={{ color: 'var(--gold-primary)', fontSize: '1.05rem', margin: 0 }}>↩️ ການຄືນສິນຄ້າ / ຄືນເງິນ (Returns &amp; Refunds)</h3>`,
    `<ReportSectionHeader icon={${makeIconJSX('returns')}} title="ການຄືນສິນຄ້າ / ຄືນເງິນ (Returns & Refunds)" />`
  );

  // Header 5: Top Selling Online
  content = content.replace(
    `<h4 style={{ color: 'var(--gold-primary)', margin: 0 }}>🏆 ສິນຄ້າຂາຍດີທາງ Online (Top Selling Products)</h4>`,
    `<ReportSectionHeader icon={${makeIconJSX('topOnline')}} title="ສິນຄ້າຂາຍດີທາງ Online (Top Selling Products)" />`
  );

  // Header 6: Online Orders
  content = content.replace(
    `<h4 style={{ color: 'var(--gold-primary)', margin: 0 }}>📋 ລາຍການ Online Orders ທັງໝົດ</h4>`,
    `<ReportSectionHeader icon={${makeIconJSX('ordersOnline')}} title="ລາຍການ Online Orders ທັງໝົດ" />`
  );

  // Header 7: Revenue Trend
  content = content.replace(
    `<h4 style={{ color: 'var(--gold-primary)', margin: 0 }}>📈 ແນວໂນ້ມລາຍຮັບ / Revenue Trend</h4>`,
    `<ReportSectionHeader icon={${makeIconJSX('trend')}} title="ແນວໂນ້ມລາຍຮັບ / Revenue Trend" />`
  );

  // Header 8: Category Sales
  content = content.replace(
    `<h4 style={{ color: 'var(--gold-primary)', margin: 0 }}>📊 ສັດສ່ວນຍອດຂາຍສິນຄ້າ / Category Sales</h4>`,
    `<ReportSectionHeader icon={${makeIconJSX('category')}} title="ສັດສ່ວນຍອດຂາຍສິນຄ້າ / Category Sales" />`
  );

  // Header 9: POS vs Online
  content = content.replace(
    `<h4 style={{ color: 'var(--gold-primary)', margin: 0 }}>📊 ສັດສ່ວນຍອດຂາຍ POS vs Online</h4>`,
    `<ReportSectionHeader icon={${makeIconJSX('channel')}} title="ສັດສ່ວນຍອດຂາຍ POS vs Online" />`
  );

  // Header 10: Payment Breakdown
  content = content.replace(
    `<h4 style={{ color: 'var(--gold-primary)', margin: 0 }}>💳 ສະຫຼຸບການຊຳລະ (ລວມທຸກຊ່ອງທາງ)</h4>`,
    `<ReportSectionHeader icon={${makeIconJSX('payment')}} title="ສະຫຼຸບການຊຳລະ (ລວມທຸກຊ່ອງທາງ)" />`
  );

  // Header 11: Expenses Summary
  content = content.replace(
    `<h4 style={{ color: 'var(--gold-primary)', margin: 0 }}>💸 ລາຍຈ່າຍ (Expenses)</h4>`,
    `<ReportSectionHeader icon={${makeIconJSX('expenses')}} title="ລາຍຈ່າຍ (Expenses)" />`
  );

  // Header 12: System Alerts
  content = content.replace(
    `<h4 style={{ color: 'var(--gold-primary)', margin: 0, fontSize: '0.9rem' }}>🔔 ແຈ້ງເຕືອນລະບົບ (System Alerts)</h4>`,
    `<ReportSectionHeader icon={${makeIconJSX('alerts')}} title="ແຈ້ງເຕືອນລະບົບ (System Alerts)" />`
  );

  // Header 13: Quick Actions
  content = content.replace(
    `<h4 style={{ color: 'var(--gold-primary)', margin: 0, fontSize: '0.9rem' }}>⚡ ຈັດການດ່ວນ (Quick Actions)</h4>`,
    `<ReportSectionHeader icon={${makeIconJSX('quick')}} title="ຈັດການດ່ວນ (Quick Actions)" />`
  );

  // Header 14: Live Activity
  content = content.replace(
    `<h4 style={{ color: 'var(--gold-primary)', margin: 0, fontSize: '0.9rem' }}>🕒 ປະຫວັດເຄື່ອນໄຫວ (Live Activity)</h4>`,
    `<ReportSectionHeader icon={${makeIconJSX('activity')}} title="ປະຫວັດເຄື່ອນໄຫວ (Live Activity)" />`
  );

  // Header 15: Treat History
  content = content.replace(
    `<h4 style={{ color: 'var(--gold-primary)', margin: 0, fontSize: '1.02rem' }}>📋 ລາຍລະອຽດການລ້ຽງແຂກ (Treat History)</h4>`,
    `<ReportSectionHeader icon={${makeIconJSX('treat')}} title="ລາຍລະອຽດການລ້ຽງແຂກ (Treat History)" />`
  );
