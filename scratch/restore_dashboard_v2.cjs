...
if (fs.existsSync(jsonPath)) {
  ...
  // 1. Modify fmtS currency formatter
  ...
  // 3. Modify Dashboard function parameters & getRange setup
  ...
  // 4. Update Header Render with Custom Presets & Inputs
  ...
  // 5. Replace PRIMARY KPI GRID and SECONDARY KPIs
  const startMarker = '          {/* ═══════════════════════\n              PRIMARY KPI GRID\n          ═══════════════════════ */}';
  const endMarker = '          {/* ═══════════════════════\n              MAIN GRID (2 columns)\n          ═══════════════════════ */}';
  ...
  if (startIdx !== -1 && endIdx !== -1) {
    const originalGridSegment = code.substring(startIdx, endIdx);
    const replacementGridSegment = `${startMarker}
    ...
    code = code.replace(originalGridSegment, replacementGridSegment);
    console.log("✓ Successfully replaced KPI primary and secondary grids!");
  } else {
    console.log("Failed to match grid start and end markers.");
  }

  fs.writeFileSync(dashboardPath, code, 'utf8');
  console.log("✓ Done restoring Dashboard.jsx!");
}
