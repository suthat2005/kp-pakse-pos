...
commits.forEach(c => {
  try {
    const show = execSync(`git show --oneline -s ${c}`).toString('utf8').trim();
    const stat = execSync(`git show --name-only --oneline ${c}`).toString('utf8').trim();
    console.log(`=== Commit ${c.slice(0, 7)} ===`);
    console.log(show);
    console.log(stat);
  } catch(e) {
    console.error(`Error showing ${c}:`, e.message);
  }
});
